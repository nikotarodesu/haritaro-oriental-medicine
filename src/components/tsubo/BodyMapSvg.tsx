...
export default function BodyMapSvg({
  ...
  const regions: { id: BodyPart; label: string; count?: number }[] = [
    { id: "頭部・顔面", label: "頭部・顔面", count: pointCountsByPart["頭部・顔面"] || 52 },
    { id: "首・肩", label: "首・肩", count: pointCountsByPart["首・肩"] || 34 },
    { id: "胸・腹", label: "胸・腹", count: pointCountsByPart["胸・腹"] || 46 },
    { id: "背中・腰", label: "背中・腰", count: pointCountsByPart["背中・腰"] || 68 },
    { id: "手・腕", label: "手・腕", count: pointCountsByPart["手・腕"] || 73 },
    { id: "足・脚", label: "足・脚", count: pointCountsByPart["足・脚"] || 88 },
  ];
        ...
        <div className="md:col-span-5 flex justify-center py-1">
          <div className="relative w-[180px] sm:w-[200px] h-[280px] sm:h-[300px] bg-[#FAF8F5] dark:bg-[#10171F] rounded-2xl border border-[#E8E1D1] dark:border-[#22303D] p-2 flex justify-center items-center shadow-inner select-none">
            ...
            <svg viewBox="0 0 160 250" className="w-full h-full">
              {/* 頭部・顔面 */}
              <g
                onClick={() => onSelectBodyPart("頭部・顔面")}
              ...
              {/* 手・腕（両側） */}
              <g
                onClick={() => onSelectBodyPart("手・腕")}
                className="cursor-pointer group"
              >
                {/* 左上肢（向かって左） */}
                <path
                  d="M 38,65 L 48,65 L 32,135 L 22,135 Z"
                  className={`transition-all duration-200 ${
              ...
              {/* 骨盤部 */}
              <path d="M 55,122 L 105,122 L 95,138 L 65,138 Z" fill="#D0C7B6" opacity="0.6" />

              {/* 足・脚（両側） */}
              <g
                onClick={() => onSelectBodyPart("足・脚")}
                className="cursor-pointer group"
              >
                {/* 左下肢 */}
                <path
          ...
          {regions.map((region) => {
            ...
            return (
              <button
                key={region.id}
                type="button"
                onClick={() => onSelectBodyPart(isSelected ? "すべて" : region.id)}
