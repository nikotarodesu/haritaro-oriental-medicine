...
export default function YinYangDisharmonyMap() {
  ...
  return (
    <figure className="my-8 bg-[#FFFFFF] dark:bg-[#17212A] rounded-3xl border border-[#E5DEC9] dark:border-[#2A3B4A] p-6 sm:p-8 shadow-sm transition-colors">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-[#F2ECE0] dark:border-[#22303D] pb-4 mb-6">
        <div>
          <span className="text-[11px] font-bold text-[#1E3D34] dark:text-[#74BA9E] uppercase tracking-wider flex items-center gap-1.5">
            ...
            <span>画像解説④：陰陽失調の4象限バランスマップ（Water & Fire Balance Map）</span>
          </span>
      ...
      {/* 基準説明バー */}
      <div className="mb-6 p-4 rounded-2xl bg-[#FAF8F5] dark:bg-[#121920] border border-[#E8E1D1] dark:border-[#22303D] flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
        <div className="flex items-center gap-2">
          <span className="w-3 h-3 rounded-full bg-[#1E3D34] shrink-0" />
          <strong className="text-[#232826] dark:text-[#FAF8F5]">健康（動的平衝）の基準：</strong>
          <span className="text-[#59615D] dark:text-[#96A6B2]">「火（陽熱・機能）50%」と「水（陰液・潤い）50%」が常に調和</span>
        </div>
          ...
          return (
            ...
            >
              ...
              <button
                ...
                className="w-full p-4 sm:p-5 text-left flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-colors"
              >
                {/* パターン名とバッジ */}
                ...
                {/* 火と水の液面ゲージビジュアル */}
                <div className="flex items-center gap-6 sm:gap-8 bg-white dark:bg-[#15202B] px-4 py-2.5 rounded-xl border border-[#E8E1D1] dark:border-[#2A3B4A] shrink-0">
                  {/* 火（陽）ゲージ */}
                  <div className="flex items-center gap-2">
