  ...
  const phases = [
    {
      ...
      vector: "上方・四方へ伸展・成長",
      color: "#2E7D32",
      activeBorder: "border-[#2E7D32] dark:border-[#4CAF50]",
      bgLight: "bg-[#EBF5EE] border-[#C8E6C9] text-[#2E7D32]",
      darkBg: "dark:bg-[#15281B] dark:border-[#274D32] dark:text-[#81C784]",
      icon: <Leaf className="w-4 h-4 text-[#2E7D32] dark:text-[#81C784]" />,
    ...
    },
    {
      ...
      vector: "炎上・拡散・激しい熱変化",
      color: "#C62828",
      activeBorder: "border-[#C62828] dark:border-[#EF5350]",
      bgLight: "bg-[#FDECEC] border-[#FFCDD2] text-[#C62828]",
      darkBg: "dark:bg-[#2B1516] dark:border-[#532426] dark:text-[#EF5350]",
      icon: <Flame className="w-4 h-4 text-[#C62828] dark:text-[#EF5350]" />,
    ...
    },
    {
      ...
      vector: "安定・受容・変換・生化",
      color: "#F57F17",
      activeBorder: "border-[#F57F17] dark:border-[#FFB74D]",
      bgLight: "bg-[#FFF9E6] border-[#FFE082] text-[#B86924]",
      darkBg: "dark:bg-[#291F11] dark:border-[#523E1D] dark:text-[#FFB74D]",
      icon: <Mountain className="w-4 h-4 text-[#F57F17] dark:text-[#FFB74D]" />,
    ...
    },
    {
      ...
      vector: "収斂・沈降・清潔・選別",
      color: "#78909C",
      activeBorder: "border-[#78909C] dark:border-[#B0BEC5]",
      bgLight: "bg-[#F0F4F8] border-[#CFD8DC] text-[#455A64]",
      darkBg: "dark:bg-[#1A2329] dark:border-[#37474F] dark:text-[#B0BEC5]",
      icon: <Shield className="w-4 h-4 text-[#78909C] dark:text-[#B0BEC5]" />,
    ...
    },
    {
      ...
      color: "#1E40AF",
      activeBorder: "border-[#1E40AF] dark:border-[#60A5FA]",
      bgLight: "bg-[#EFF6FF] border-[#BFDBFE] text-[#1E40AF]",
      darkBg: "dark:bg-[#132038] dark:border-[#1E40AF] dark:text-[#93C5FD]",
      icon: <Droplets className="w-4 h-4 text-[#1E40AF] dark:text-[#60A5FA]" />,
          ...
          <div className="relative w-full max-w-[340px] aspect-square flex items-center justify-center">
            <svg viewBox="0 0 340 340" className="w-full h-full drop-shadow-md">
              ...
              <g transform="translate(170, 170)">
                ...
                <text x="0" y="38" textAnchor="middle" fill="currentColor" className="text-[#59615D] dark:text-[#CFD8DC] text-[9px] font-bold">太極（根源）</text>
              </g>

              {/* 放射状のエネルギー分化ライン */}
              {/* 木（左） */}
              <line x1="145" y1="160" x2="60" y2="150" stroke="currentColor" className="text-[#2E7D32] dark:text-[#4ADE80]" strokeWidth="2" strokeDasharray="3 2" />
              {/* 火（上） */}
              <line x1="170" y1="140" x2="170" y2="55" stroke="currentColor" className="text-[#C62828] dark:text-[#F87171]" strokeWidth="2" strokeDasharray="3 2" />
              {/* 土（右中） */}
              <line x1="195" y1="175" x2="270" y2="170" stroke="currentColor" className="text-[#F57F17] dark:text-[#FBBF24]" strokeWidth="2" strokeDasharray="3 2" />
              {/* 金（右下） */}
              <line x1="185" y1="190" x2="245" y2="265" stroke="currentColor" className="text-[#78909C] dark:text-[#94A3B8]" strokeWidth="2" strokeDasharray="3 2" />
              {/* 水（左下） */}
              <line x1="155" y1="190" x2="95" y2="265" stroke="currentColor" className="text-[#1E40AF] dark:text-[#60A5FA]" strokeWidth="2" strokeDasharray="3 2" />

              {/* 五行の5ノード */}
              {/* 火：上（夏・太陽） */}
              <g transform="translate(170, 45)" className="cursor-pointer" onClick={() => setActiveElement("fire")}>
                <circle cx="0" cy="0" r="24" fill="currentColor" className="text-[#C62828] dark:text-[#EF4444]" />
                <text x="0" y="4" textAnchor="middle" fill="#FFF" fontSize="13" fontWeight="bold">火</text>
                <text x="0" y="-30" textAnchor="middle" fill="currentColor" className="text-[#C62828] dark:text-[#FCA5A5] text-[10px] font-bold">太陽（夏）</text>
              </g>

              {/* 土：中央右（中和・土用） */}
              <g transform="translate(285, 160)" className="cursor-pointer" onClick={() => setActiveElement("earth")}>
                <circle cx="0" cy="0" r="24" fill="currentColor" className="text-[#F57F17] dark:text-[#F59E0B]" />
                <text x="0" y="4" textAnchor="middle" fill="#FFF" fontSize="13" fontWeight="bold">土</text>
                <text x="0" y="36" textAnchor="middle" fill="currentColor" className="text-[#B86924] dark:text-[#FCD34D] text-[10px] font-bold">中和（土用）</text>
              </g>

              {/* 金：右下（秋・少陰） */}
              <g transform="translate(250, 275)" className="cursor-pointer" onClick={() => setActiveElement("metal")}>
                <circle cx="0" cy="0" r="24" fill="currentColor" className="text-[#78909C] dark:text-[#64748B]" />
                <text x="0" y="4" textAnchor="middle" fill="#FFF" fontSize="13" fontWeight="bold">金</text>
                <text x="0" y="36" textAnchor="middle" fill="currentColor" className="text-[#455A64] dark:text-[#CBD5E1] text-[10px] font-bold">少陰（秋）</text>
              </g>

              {/* 水：左下（冬・太陰） */}
              <g transform="translate(90, 275)" className="cursor-pointer" onClick={() => setActiveElement("water")}>
                <circle cx="0" cy="0" r="24" fill="currentColor" className="text-[#1E40AF] dark:text-[#3B82F6]" />
                <text x="0" y="4" textAnchor="middle" fill="#FFF" fontSize="13" fontWeight="bold">水</text>
                <text x="0" y="36" textAnchor="middle" fill="currentColor" className="text-[#1E40AF] dark:text-[#93C5FD] text-[10px] font-bold">太陰（冬）</text>
              </g>

              {/* 木：左（春・少陽） */}
              <g transform="translate(50, 145)" className="cursor-pointer" onClick={() => setActiveElement("wood")}>
                <circle cx="0" cy="0" r="24" fill="currentColor" className="text-[#2E7D32] dark:text-[#22C55E]" />
                <text x="0" y="4" textAnchor="middle" fill="#FFF" fontSize="13" fontWeight="bold">木</text>
                <text x="0" y="-30" textAnchor="middle" fill="currentColor" className="text-[#2E7D32] dark:text-[#86EFAC] text-[10px] font-bold">少陽（春）</text>
              </g>
            </svg>
          </div>
        </div>
              ...
              return (
                <div
                  ...
                  className={`p-3 rounded-xl border transition-all cursor-pointer ${
                    activeElement === p.id
                      ? `ring-2 ring-offset-1 bg-white dark:bg-[#1C2834] shadow-sm ${p.activeBorder}`
                      : "bg-white/80 dark:bg-[#17212A] hover:bg-white border-[#E8E1D1] dark:border-[#22303D]"
                  }`}
                >
