  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      if (lectureId) {
        // 後方互換：旧ID対応
        const resolvedId =
          lectureId === "lecture-1" || lectureId === "lecture-1-yinyang"
            ? "lecture-yinyang-1"
            : lectureId;
          ...
          <div className="bg-[#FFFFFF] dark:bg-[#17212A] rounded-2xl border border-[#E5DEC9] dark:border-[#2A3B4A] p-2.5 sm:p-4 shadow-2xs">
            ...
            <div className="grid grid-cols-4 sm:grid-cols-8 gap-1.5 text-center text-[10px]">
              {[
                ...
                { num: 2, label: "② 五行", targetId: "lecture-2-wuxing" },
              ...
              <div className="flex items-center justify-between">
                ...
                {activeLecture.seriesTitle && activeLecture.lessonNumber && (
                  <span className="text-[11px] font-bold px-2.5 py-0.5 rounded-full bg-[#1E3D34] text-white">
                    {activeLecture.seriesTitle} 第{activeLecture.lessonNumber}章 / 全8章
                  </span>
                )}
              </div>
    ...
    <div className="max-w-7xl mx-auto px-2.5 sm:px-6 lg:px-8 py-8 sm:py-16 space-y-10 sm:space-y-12">
      ...
      <div className="space-y-8 sm:space-y-12">
        {CURRICULUM_DATA.map((stage) => {
          // 陰陽論以外の講義を抽出
          const stageLectures = stage.lectures.filter((l) => l.seriesId !== "yinyang");
